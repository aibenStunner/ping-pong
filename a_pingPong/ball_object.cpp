...
glm::vec2 BallObject::Move(float dt, unsigned int window_height, bool isPlayer1)
{
	// if not stuck to player board
	if (!this->Stuck)
	{
		// move the ball
		if (isPlayer1) this->Position -= this->Velocity * dt; 
		else this->Position += this->Velocity * dt;
